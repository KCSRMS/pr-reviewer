...
// on the pull request. ref is the pull request head SHA. Missing files and
// GitHub errors are skipped.
func GatherPromptExtras(ctx context.Context, client github.Client, owner, repo string, number int, ref string) (rules, existing string) {
	if client == nil {
		return "", ""
	}
	return loadRepoRules(ctx, client, owner, repo, ref), loadExistingComments(ctx, client, owner, repo, number)
}

func loadRepoRules(ctx context.Context, client github.Client, owner, repo, ref string) string {
	var b strings.Builder
	for _, path := range repoRulePaths {
		var content string
		var err error
		if ref != "" {
			content, _, err = client.GetFileContentAtRef(ctx, owner, repo, path, ref)
		} else {
...
func loadExistingComments(ctx context.Context, client github.Client, owner, repo string, number int) string {
	comments, err := client.ListReviewComments(ctx, owner, repo, number, existingCommentLimit)
